...
@Component
public class LorAppointmentCommand extends DoctorsAppointmentCommand implements ProcessCommand {
    SendMessage message = new SendMessage();

    @Override
    public SendMessage start(Update update) {
        if (!update.getMessage().getText().equals("Отоларинголог")) {
            return null;
        }
        ...
        appointment.setDoctor(Doctors.LOR);
    ...
    public SendMessage sendDefaultMessage(Update update) {
        ...
        List<String> appointmentTime = AppointmentHelper.getFreeIntervals(Doctors.LOR);
